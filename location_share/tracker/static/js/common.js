...
function retryQRGeneration() {
    ...
    if (qrContainer) {
        ...
        setTimeout(() => {
            // generateQRCodeAlternative関数を呼び出す
            if (typeof generateQRCodeAlternative === 'function') {
...
function shareToSocial(platform) {
    ...
    if (navigator.share && platform === 'native') {
        navigator.share({
            title: title,
            text: 'ちょこシェアMAPアプリをチェックしてみてください！',
            url: url
        }).then(() => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    socialButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            switch(socialType) {
                case 'twitter':
                    shareUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent('https://locationsshare.com')}&hashtags=ちょこシェアMAP,ブラウザ動作,登録不要`;
                    //showCustomAlert('Twitterの共有ページを開きました', 'Twitter共有', 'success');
                    break;

                case 'facebook':
                    shareUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent('https://locationsshare.com')}&quote=${encodeURIComponent(description)}`;
    ...
    // QR再試行ボタンのイベント設定
    const retryButtons = document.querySelectorAll('[data-action="retry-qr"]');
